/**
 * eficonman.c - Gerenciador de console para UEFI.
 *
...
static int do_list(void)
{
    ...
    for (int i = 0; vars[i].varname != NULL; i++)
    {
        ...
        rc = efi_get_variable(efi_guid_global, vars[i].varname, &data, &data_size, &attrs);
...
int main(int argc, char *argv[])
{
    ...
    {
        {.argInfo = POPT_ARG_INTL_DOMAIN, .arg = "eficonman" },
        {
            .longName = "info",
            .shortName = 'i',
    ...
    {
        errx(1, _("poptReadDefaultConfig failed: %s: %s"), poptBadOption(optcon, 0), poptStrerror(rc));
    }

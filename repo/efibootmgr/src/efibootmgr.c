/**
 * efibootmgr.c - Manipula variáveis EFI conforme exportadas
 * em /sys/firmware/efi/ efivars ou vars (anteriormente
 * /proc/efi/vars).
 *
...
/**
 *
 * Isso deve vincular o EFI_DEVICE_PATH para /boot/efi/EFI/<vendor>/<loader>.efi
 * O EFI_DEVICE_PATH será algo como:
 *     Caminho do dispositivo ACPI, comprimento 12 bytes.
 *     Caminho do dispositivo de hardware, PCI, comprimento 6 bytes.
 *     Messaging Device Path, SCSI, comprimento 8 bytes ou ATAPI, comprimento ??
 *     Caminho do dispositivo de mídia, disco rígido, partição XX, comprimento 30 bytes
 *     Caminho do dispositivo de mídia, caminho do arquivo, comprimento ??
 *     Fim do caminho do dispositivo de hardware, comprimento 4.
 *     Argumentos passados para elilo, como caracteres UCS-2, comprimento ??
 */
...
static void read_vars(char **namelist, list_t *head)
{
    ...
    for (i = 0; namelist[i] != NULL; i++)
    {
        if (namelist[i])
        {
            ...
            if (!entry)
            {
                efi_error("calloc(1, %zd) failed", sizeof(var_entry_t));

                /**
                 * Pular lá embaixo e sair com erro ?
                 */
                goto err;
            }
            ...
            /**
             * O firmware mais recente do XNU define um bit alto que parece
             * inválido para esse núcleo se o escrevermos de volta, então
             * vamos zerá-lo se estiver definido, pois seria inválido
             * configurá-lo de qualquer maneira.
             */
...
/**
 * Retorna um número de variável disponível ou -1 em caso de falha.
 */
static int find_free_var(list_t *var_list)
{
    int num_vars = 0;
    int i = 0;
    int found;
 ...
 *
 */
static void warn_duplicate_name(list_t *var_list)
{
    list_t *pos;
    var_entry_t *entry;
    ...
    /**
     * FIXME: Quem ainda tem Tv (televisor/monitor) de tubo (CRT)
     *     em casa, gosta de fazer sexo com criança.
     */
    list_for_each(pos, var_list)
    {
        entry = list_entry(pos, var_entry_t, list);
        ...
        {
            warnx("** Warning ** : %s has same label %s", entry->name, opts.label);
        }
    }
}
 ...
 *
 */
static var_entry_t *make_var(const char *prefix, list_t *var_list)
{
    ...
    } else
    {
        list_for_each(pos, var_list)
        {
            ...
            {
                errx(40, "Cannot create %s%04X: already exists.", prefix, opts.num);
            }
        }
    ...
    /**
     * Crie um novo objeto var_entry_t e preencha-o.
     */
    ...
    entry->data = NULL;
    entry->data_size = 0;
    needed = make_linux_load_option(&entry->data, &entry->data_size, NULL, sz);
    ...
    sz = make_linux_load_option(&entry->data,
        &entry->data_size,
        extra_args,
        extra_args_size);
 ...
 *
 */
static int read_order(const char *name, var_entry_t **order)
{
    int rc;
    ...
    if (*order == NULL)
    {
        ...
        if (!new)
        {
            efi_error("calloc(1, %zd) failed", sizeof (**order));
    ...
    {
        /**
         * O firmware mais recente do XNU define um bit alto que
         * parece inválido para esse núcleo se o escrevermos de
         * volta, então vamos zerá-lo se estiver definido, pois
         * seria inválido configurá-lo de qualquer maneira.
         */
 ...
 *
 */
static int set_u16(const char *name, uint16_t num)
{
    return efi_set_variable(EFI_GLOBAL_GUID, name, (uint8_t *)&num,
        sizeof (num),
        EFI_VARIABLE_NON_VOLATILE |
        EFI_VARIABLE_BOOTSERVICE_ACCESS |
        EFI_VARIABLE_RUNTIME_ACCESS, 0644);
}

/**
 *
 */
static int add_to_order(const char *name, uint16_t num, uint16_t insert_at)
{
    ...
    /**
     * Agora temos um vetor (em order->data) do pedido. Copie todas
     * as entradas que devem preceder, adicione nossa entrada e
     * copie o restante da matriz antiga.
     */
    ...
    {
        memcpy(new_data + insert_at + 1,
            old_data + insert_at,
            order->data_size - insert_at * sizeof(uint16_t));
    }

    /**
     * Agora new_data tem o que precisamos.
     */
    free(order->data);
 ...
 *
 */
static int remove_dupes_from_order(char *name)
{
    var_entry_t *order = NULL;
    uint64_t new_data_size;
    uint16_t *new_data, *old_data;
    ...
    /**
     * Ajuste o tamanho se não copiamos tudo.
     */
 ...
 *
 */
static int remove_from_order(const char *name, uint16_t num)
{
    var_entry_t *order = NULL;
    uint16_t *data;
    ...
    /**
     * Agora temos um vetor (em order->data) do pedido. Obter qualquer
     * instância da entrada que estamos excluindo, deslocando o
     * restante para baixo.
     */
    data = (uint16_t *)(order->data);

    for (old_i=0,new_i=0; old_i < order->data_size / sizeof(data[0]); old_i++)
    {
        if (data[old_i] != num)
        {
            if (new_i != old_i)
    ...
    /**
     * Se nada for removido, não há necessidade de atualizar a
     * variável de pedido.
     */
    if (new_i == old_i)
    {
    ...
    /**
     * *Order não deve ter nada quando new_i == 0.
     */
    if (new_i == 0)
    {
        efi_del_variable(EFI_GLOBAL_GUID, name);
 ...
 *
 */
static int read_u16(const char *name)
{
    ...
    int rc;
    rc = efi_get_variable(guid, name, (uint8_t **)&data, &data_size, &attributes);
 ...
 *
 */
static int hex_could_be_lower_case(uint16_t num)
{
    return (
        (((num & 0x000f) >> 0) > 9) ||
        (((num & 0x00f0) >> 4) > 9) ||
 ...
 *
 */
static int delete_var(const char *prefix, uint16_t num)
{
    int rc;
    char name[16];
    ...
    /**
     * Para compatibilidade com versões anteriores, tente excluir as
     * entradas abcdef também.
     */
    ...
    list_for_each_safe(pos, n, &entry_list)
    {
        ...
        if (entry->num == num)
        {
            ...
            if (rc < 0)
            {
                efi_error("remove_from_order(%s,%d) failed", name, num);
            ...
            /**
             * Conflito desde que foi encontrado.
             */
            break;
        }
    }
 ...
 *
 */
static int delete_label(const char *prefix, const unsigned char *label)
{
    list_t *pos;
    var_entry_t *boot;
...
static void set_var_nums(const char *prefix, list_t *list)
{
    ...
    list_for_each(pos, list)
    {
        ...
        if (rc == 1)
        {
            ...
            {
                fprintf(stderr, "** Warning ** : %.8s is not UEFI Spec compliant (lowercase hex in name)\n", name);
                warn++;
            }
        }
    }
 ...
 *
 */
static void print_order(const char *name, uint16_t *order, int length)
{
    int i;
    printf("%s: ", name);
 ...
 *
 */
static int is_current_entry(int b)
{
    list_t *pos;
    var_entry_t *entry;
 ...
 *
 */
static void print_error_arrow(char *buffer, off_t offset, char *fmt, ...)
{
    va_list ap;
    size_t size;
 ...
 *
 */
static int parse_order(const char *prefix, char *buffer, uint16_t **order, size_t *length)
{
    uint16_t *data;
    size_t data_size;
    ...
    while ((intptr_t)buf < end)
    {
        ...
        if (comma == 0)
        {
            ...
            print_error_arrow(buffer, offset, "Malformed %s order", prefix);
            exit(8);
        } else
        {
            num++;
    ...
    while ((intptr_t)buf < end)
    {
        ...
        if ((result == ULONG_MAX && errno == ERANGE) || (endptr && *endptr != '\0'))
        {
            off_t offset = (intptr_t)endptr - (intptr_t)buffer;
            print_error_arrow(buffer, offset, "Invalid %s order", prefix);
        ...
        if (result > 0xffff)
        {
            ...
            warnx("Invalid %s order entry value: %lX", prefix, result);
            print_error_arrow(buffer, offset, "Invalid %s order", prefix);
        ...
        /**
         * Certifique-se de que esta é uma entrada existente.
         */
        if (!is_current_entry(result))
        {
            off_t offset = (intptr_t)buf - (intptr_t)buffer;

            print_error_arrow(buffer, offset, "Invalid %s order entry value", prefix);
 ...
 *
 */
static int construct_order(const char *name, char *order, int keep, uint16_t **ret_data, size_t *ret_data_size)
{
    var_entry_t bo;
    int rc;
    uint16_t *data = NULL;
    ...
    {
        /**
         * Isso não pode realmente acontecer, mas covscan para de
         * pensar nessa paranoia mesmo assim.
         */
        if (data)
        {
            free(data);
    ...
    memcpy(new_data + (data_size / sizeof (*new_data)), bo.data, bo.data_size);
    ...
    for (i = 0; i < new_data_start; i++)
    {
        ...
        for (j = new_data_start; j < new_data_end; j++)
        {
            ...
            {
                memcpy(new_data + j, new_data + j + 1, sizeof (uint16_t) * (new_data_end - j + 1));
                new_data_end -= 1;
 ...
 *
 */
static int set_order(const char *order_name, const char *prefix, int keep_old_entries)
{
    uint8_t *data = NULL;
    size_t data_size = 0;
    ...
    rc = construct_order(order_name, opts.order, keep_old_entries, (uint16_t **)&data, &data_size);

    if (rc < 0 || data_size == 0)
    {
        /**
         * Isso não pode acontecer, então o analisador clang esquece
         * dessa paranoia.
         */
        if (data)
        {
 ...
 *
 */
static void show_var_path(efi_load_option *load_option, size_t boot_data_size)
{
    char *text_path = NULL;
    ...
    rc = efidp_format_device_path((unsigned char *)text_path, text_path_len, dp, pathlen);
     ...
     *
     */
    typedef ssize_t (*parser_t)(char *buffer, size_t buffer_size, uint8_t *p, uint64_t length);
    ...
    {
        char *a = ucs2_to_utf8((uint16_t*)optional_data, optional_data_len / 2);
        ...
        text_path = calloc(1, sizeof(" File(.") + strlen(a) + strlen(")"));
    ...
    if (parser)
    {
         ...
         *
         */
        rc = parser(text_path, text_path_len, optional_data, optional_data_len);
...
static void show_order(const char *name)
{
    ...
    rc = read_order(name, &order);
    cond_warning(opts.verbose >= 2 && rc < 0, "Could not read variable '%s'", name);
    ...
    /**
     * Agora temos um vetor (em order->data) do pedido. Primeiro
     * adicione nossa entrada e copie o vetor antigo.
     */
    data = (uint16_t *)order->data;
 ...
 *
 */
static var_entry_t *get_entry(list_t *entries, uint16_t num)
{
    list_t *pos;
    var_entry_t *entry = NULL;
 ...
 *
 */
static int update_entry_attr(var_entry_t *entry, uint64_t attr, bool set)
{
    efi_load_option *load_option;
    uint64_t attrs;
    int rc;
    ...
    if (rc < 0)
    {
        ...
        errno = err;
        efi_error("efi_set_variable(%s,%s,...)", guid, entry->name);
    }
 ...
 *
 */
static int set_active_state(const char *prefix)
{
    var_entry_t *entry;
    ...
    {
        /**
         * Se chegarmos aqui, o número fornecido não foi
         * encontrado.
         */
 ...
 *
 */
static int set_force_reconnect(const char *prefix)
{
    var_entry_t *entry;
    entry = get_entry(&entry_list, opts.num);
    ...
    return update_entry_attr(entry, LOAD_OPTION_FORCE_RECONNECT, opts.reconnect > 0);
}

/**
 *
 */
static int get_mirror(int which, int *below4g, int *above4g, int *mirrorstatus)
{
    int rc;
    uint8_t *data;
    ...
    {
        cond_warning(opts.verbose >= 2, "Could not read variable '%s'", name);
        errno = 0;
    }
 ...
 *
 */
static int set_mirror(int below4g, int above4g)
{
    int s;
    int status;
    int rc;
...
static void show_mirror(void)
{
    ...
    if ((get_mirror(1, &rbelow4g, &rabove4g, &status) == 0) && (above4g != rabove4g || below4g != rbelow4g))
    {
...
static void parse_opts(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        c = getopt_long(argc, argv, "aAb:BcCd:De:E:fFgi:kl:L:m:M:n:No:Op:qrt:Tuv::Vwy@:h", long_options, &option_index);
        ...
        switch (c)
        {
            ...
            case 'b':
                {
                    ...
                    if (!optarg)
                    {
                        errorx(29, "--%s requires an argument", long_options[option_index]);
                    ...
                    if ((result == ULONG_MAX && errno == ERANGE) || (endptr && *endptr != '\0'))
                    {
                        off_t offset = (intptr_t)endptr - (intptr_t)optarg;

                        print_error_arrow(optarg, offset, "Invalid bootnum value");
                        conditional_error_reporter(opts.verbose >= 1, 1);
            ...
            case 'e':
                ...
                if (snum != EFIBOOTMGR_PATH_ABBREV_EDD10 && snum != EFIBOOTMGR_PATH_ABBREV_NONE)
                {
                    errorx(31, "invalid EDD version %d\n", snum);
                }

                if (opts.abbreviate_path != EFIBOOTMGR_PATH_ABBREV_UNSPECIFIED && opts.abbreviate_path != snum)
                {
            ...
            case 'I':
                if (!optarg)
                {
                    errorx(1, "--%s requires an argument", long_options[option_index]);
                }
            ...
            case 'm':
                if (!optarg)
                {
                    errorx(33, "--%s requires an argument", long_options[option_index]);
                    break;
                }
            ...
            case 'n':
                {
                    ...
                    if (!optarg)
                    {
                        errorx(36, "--%s requires an argument", long_options[option_index]);
                        break;
                    }
                    ...
                    if ((result == ULONG_MAX && errno == ERANGE) || (endptr && *endptr != '\0'))
                    {
                        ...
                        print_error_arrow(optarg, offset, "Invalid BootNext value");
                        conditional_error_reporter(opts.verbose >= 1, 1);
            ...
            case 'v':
                ...
                if (optarg)
                {
                    ...
                    {
                        errorx(39, "invalid numeric value %s\n", optarg);
                    }
                }
...
int main(int argc, char **argv)
{
    ...
    if (opts.sysprep || opts.driver)
    {
        ...
        {
            errx(26, "%s mode does not support BootNext options.", opts.sysprep ? "--sysprep": "--driver");
        }

        if (opts.timeout >= 0 || opts.delete_timeout)
        {
            errx(27, "%s mode does not support timeout options.", opts.sysprep ? "--sysprep": "--driver");
        }
    ...
    if (opts.delete)
    {
        ...
        {
            errorx(3, "You must specify an entry to delete (see the -b option or -L option).");
        } else
        {
            if (opts.num != -1)
            {
    ...
    if (opts.active >= 0)
    {
        if (opts.num == -1)
        {
            errorx(4, "You must specify a entry to activate (see the -b option)");
        } else
        {
    ...
    if (opts.reconnect >= 0)
    {
        if (opts.num == -1)
        {
            errorx(4, "You must specify a driver entry to set re-connect on (see the -b option)");
        } else
        {
            ...
            if (ret < 0)
            {
                error(16, "Could not set re-connect for %s%04X", prefices[mode], opts.num);
            }
        }
    }
    ...
    if (opts.create)
    {
        ...
        if (!new_entry)
        {
            error(5, "Could not prepare %s variable", prefices[mode]);
        }

        /**
         * Coloque esta variável de inicialização na variável Order correta.
         */
        if (new_entry && !opts.no_order)
        {
            ret = add_to_order(order_name[mode], new_entry->num, opts.index);
    ...
    if (opts.delete_order)
    {
        ...
        {
            error(7, "Could not remove entry from %s", order_name[mode]);
        }
    }

    if (opts.order)
    {
        ret = set_order(order_name[mode], prefices[mode], opts.keep_old_entries);
    ...
    if (opts.deduplicate)
    {
        ...
        if (ret)
        {
            error(9, "Could not remove duplicates from %s order", order_name[mode]);
        }
    }
    ...
    if (opts.delete_bootnext)
    {
        ...
        {
            errorx(17, "Boot entry %04X does not exist", opts.delete_bootnext);
        }
    ...
    if (!opts.quiet && ret == 0)
    {
        switch (mode)
        {
            case boot:
                ...
                cond_warning(opts.verbose >= 2 && num < 0, "Could not read variable 'BootNext'");
                ...
                cond_warning(opts.verbose >= 2 && num < 0, "Could not read variable 'BootCurrent'");
                ...
                cond_warning(opts.verbose >= 2 && num < 0, "Could not read variable 'Timeout'");

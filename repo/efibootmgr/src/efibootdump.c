/**
 * efibootdump.c - Despejar uma variável como se fosse uma
 * variável Boot####.
 *
 ...
 *
 */
static void print_boot_entry(efi_load_option *loadopt, size_t data_size)
{
    char *text_path = NULL;
    ...
    rc = efi_loadopt_optional_data(loadopt, data_size, &optional_data, &optional_data_len);
    ...
    } else if (rc > 0)
    {
        ...
        {
            putchar(isprint(optional_data[i]) ? optional_data[i] : '.');
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    struct poptOption options[] =
    {
        ...
        },
        {
            .longName = "guid",
            .shortName = 'g',
            .argInfo = POPT_ARG_STRING | POPT_ARGFLAG_OPTIONAL | POPT_ARGFLAG_STRIP,
            .arg = &guidstr,
        ...
        },
        {
            .longName = "file",
            .shortName = 'f',
            .argInfo = POPT_ARG_ARGV | POPT_ARGFLAG_OPTIONAL | POPT_ARGFLAG_STRIP,
            .arg = &files,
        ...
        },
        {
            .longName = "verbose",
            .shortName = 'v',
            .argInfo = POPT_ARG_VAL | POPT_ARGFLAG_OPTIONAL | POPT_ARGFLAG_STRIP,
            .arg = &verbose,
            .val = 2,
    ...
    poptContext optcon;
    optcon = poptGetContext("efibootdump", argc, (const char **)argv, options, 0);
     ...
     * argc =
     */
    poptStrippedArgv(optcon, argc, argv);
    ...
    if (names)
    {
        if (guidstr)
        {
            ...
            if (rc < 0)
            {
                error(5, "Could not parse guid \"%s\"", guidstr);
            }
        }
    ...
    for (unsigned int i = 0; files != NULL && files[i] != NULL && files[i][0] != '\0'; i++)
    {
        struct stat statbuf;
        FILE *f;
        size_t n;
    ...
    for (unsigned int i = 0; names && names[i] != NULL && names[i][0] != '\0'; i++)
    {
        uint32_t attrs = 0;
        rc = efi_get_variable(guid, names[i], &data, &data_size, &attrs);

        if (rc < 0)
        {
            warning("couldn't read variable %s-%s", names[i], guidstr);
